import { PlayerName, PlaylistItem, Track, PlayerDevice, PlayerContext, getSpotifyPlayerContext } from "cody-music";
...
export class MusicDataManager {
  ...
  isLikedTrack(trackId: string) {
    const foundSong = this.spotifyLikedSongs ? this.spotifyLikedSongs.find((n: Track) => n.id === trackId) : null;
  ...
  getMusicTimePlaylistByTypeId(playlistTypeId: number): PlaylistItem {
    const pItem: PlaylistItem = this.generatedPlaylists.find((e: PlaylistItem) => e.playlistTypeId === playlistTypeId);
    return pItem;
  }
  ...
  populateGeneratedPlaylists() {
    ...
    if (this.savedPlaylists.length > 0 && this.rawPlaylists.length > 0) {
      this.savedPlaylists.forEach((savedPlaylist: PlaylistItem) => {
        ...
        const rawIdx = this.rawPlaylists.findIndex((n: PlaylistItem) => n.id === savedPlaylist.id);
        const origRawPlaylistOrderIdx = this.origRawPlaylistOrder.findIndex((n: PlaylistItem) => n.id === savedPlaylist.id);
        if (rawIdx !== -1) {
